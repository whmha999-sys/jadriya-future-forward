...
import ogLogo from "@/assets/og-logo-icon.png";
...
const OilGasNavbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const heroSection = document.getElementById("og-hero");
      if (heroSection) {
      ...
      >
        ...
        <div className={`h-8 flex items-center px-6 md:px-12 transition-colors duration-300 ${
          ...
          <button
            ...
            className={`flex items-center gap-1 text-[11px] font-medium tracking-wide transition-colors duration-300 ${
              bgSolid ? "text-accent hover:text-accent/80" : "text-accent hover:text-accent/80"
            }`}
          >
        ...
        <div className="flex items-center justify-between px-6 md:px-12 py-1.5 h-[62px]">
          {/* Logo */}
          <button onClick={() => navigateWithTransition("/oil-gas")} className="flex items-center gap-0">
            <div className="relative h-12 w-12 shrink-0">
              <img
                src={ogLogo}
                alt="AL-JADRIYA Oil & Gas"
            ...
            <div className={`hidden md:block ml-2 transition-colors duration-300 ${bgSolid ? "text-primary" : "text-primary-foreground"}`}>
              ...
              <span className="block text-[9px] font-medium tracking-[0.3em] uppercase opacity-70">{t("og.oilAndGas")}</span>
            </div>
          </button>
